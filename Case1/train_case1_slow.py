...
def local_run(x_train,x_test,y_train,y_test, local_rho, local_learning_rate, local_iter):
  ...
  layers.Dense(240,activation = 'relu', kernel_regularizer=regularizers.l2(local_rho)),
  layers.Dense(120,activation = 'relu', kernel_regularizer=regularizers.l2(local_rho)),
  layers.Dense(60,activation = 'relu', kernel_regularizer=regularizers.l2(local_rho)),
...
plt.title("Case 1 Slow - Velocity Prediction")
plt.legend(loc='lower right', fontsize='x-small')
plt.show()
np.savetxt("case1_slow_predict_velocity.txt", y_prediction)
# plt.ylim(-1,1)
